...
export function ConfirmDialog({ open, payload, onClose }: ConfirmDialogProps) {
  ...
  return (
    <Dialog maxWidth='xs' fullWidth open={open} onClose={() => onClose(false)}>
      ...
      <DialogContent>{payload.msg}</DialogContent>
      <DialogActions>
        <Button autoFocus disabled={!open} {...cancelButtonProps} variant='text'>
          {payload.cancelText ?? 'Cancel'}
        </Button>
        <Button color={payload.severity} disabled={!open} {...okButtonProps} variant='text'>
          {payload.okText ?? 'Ok'}
        </Button>
      </DialogActions>
    </Dialog>
...
export function PromptDialog({ open, payload, onClose }: PromptDialogProps) {
  ...
  return (
    ...
    >
      ...
      <DialogActions>
        <Button disabled={!open} {...cancelButtonProps} variant='text'>
          {payload.cancelText ?? 'Cancel'}
        </Button>
        <Button disabled={!open} loading={loading} type='submit' variant='text'>
          {payload.okText ?? 'Ok'}
        </Button>
      </DialogActions>
    </Dialog>
