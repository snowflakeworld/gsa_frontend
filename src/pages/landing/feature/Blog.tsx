...
import { Box, Card, CardContent, Grid, Stack, Typography, useColorScheme } from "@mui/material"
  ...
  return (
    <Grid size={{ xs: 12, md: 6 }}>
      <Card sx={{
        ...
        <CardContent sx={{
          height: '100%', padding: 0, '&:last-child': { paddingBottom: 0 }
        }}>
          <Stack gap={4}>
            <Stack direction={isScreenSmall ? 'column' : 'row'} alignItems={'center'} gap={2}>
              <Stack sx={
                {
                  borderRadius: 1, minWidth: 52, minHeight: 52, backgroundColor: gsaColors.background.red['alpha']
                }
              } justifyContent='center' alignItems='center'>
                <AppIcon name={icon} className='fill-none' width={24} height={24} viewBoxWidth={36} viewBoxHeight={36} />
              </Stack>
              <Typography variant="h4" textAlign={!isScreenSmall ? 'initial' : 'center'}
                sx={{
